...
type
  ...
  private
    ...
    FLineSpace:                        Single;        // 行后间距
    FAlignment:                        TParaAlign;    // 对齐方式
    FIndentLevel:                      Integer;       // 缩进级别
    FIsFolded:                         Boolean;       // 是否折叠
    // 文字格式
    FFont:                             Integer;       // 字体编号
    FSize:                             Single;        // 字体大小
    FOpacity:                          Single;        // 字透明度
    FBgOpacity:                        Single;        // 底透明度
    FOffset:                           Single;        // 字体偏移
    ...
    procedure SetIsFolded(const Value: Boolean);
  public
    // 方法
    ...
    property IsFolded:            Boolean        read FIsFolded                write SetIsFolded default False;
    // 字体属性
  ...
  private
    ...
    // 文档属性
    FLevelFirst:                       Single;        // 每一级提供的首行缩进
    FLevelLeft:                        Single;        // 每一级提供的左端缩进
    FLevelRight:                       Single;        // 每一级提供的右端缩进
    FFoldColor:                        TAlphaColor;   // 折叠标识颜色
    FFoldWidth:                        Single;        // 折叠标识宽度
    // 文段属性
    FPassHeight:                       Single;
    ...
    procedure SetLevelFirst(const Value: Single);
    procedure SetLevelLeft(const Value: Single);
    procedure SetLevelRight(const Value: Single);
    procedure SetFoldColor(const Value: TAlphaColor);
    procedure SetFoldWidth(const Value: Single);
      // 附属事件
  ...
  published
    ...
    property LevelFirst:          Single              read FLevelFirst    write SetLevelFirst;
    property LevelLeft:           Single              read FLevelLeft     write SetLevelLeft;
    property LevelRight:          Single              read FLevelRight    write SetLevelRight;
    property FoldLineColor:       TAlphaColor         read FFoldColor     write SetFoldColor;
    property FoldLineWidth:       Single              read FFoldWidth     write SetFoldWidth;
...
procedure TSctUI.SetIndentLevel(const Value: Integer);
var
  pi1, pi2, i: Integer;
begin
...
procedure TSctUI.SetIsFolded(const Value: Boolean);
var
  pi1, pi2, i: Integer;
begin
  FIsFolded := Value;
...
begin
  ...
  begin
    ...
    begin
      if li1 = FLineTop[pi1].Count - 1 then
        Canvas.FillRect(RectF(FLineLeft[pi1][li1], st + FLineTop[pi1][li1], FCharRight[pi2][li2][ci2 - 1],
    ...
    begin
      if li1 = FLineTop[pi1].Count - 1 then
        Canvas.FillRect(RectF(FCharRight[pi1][li1][ci1 - 1], st + FLineTop[pi1][li1],
          FCharRight[pi2][li2][ci2 - 1], st + FParaEnd[pi1]) * RectF(0, 0, Width, Height), 0, 0, [], 0.5, bb)
      else
        Canvas.FillRect(RectF(FCharRight[pi1][li1][ci1 - 1], st + FLineTop[pi1][li1],
          FCharRight[pi2][li2][ci2 - 1], st + FLineTop[pi1][li1 + 1]) * RectF(0, 0, Width, Height),
          0, 0, [], 0.5, bb);
    end;
    FreeAndNil(bb);
    Exit;
  ...
  if FParas[pi1].Count > 0 then
    ...
    begin
      if li1 = FLineTop[pi1].Count - 1 then
        Canvas.FillRect(RectF(FLineLeft[pi1][li1], st + FLineTop[pi1][li1], FCharRight[pi1][li1].Last,
          st + FParaEnd[pi1]) * RectF(0, 0, Width, Height), 0, 0, [], 0.5, bb)
      else
        Canvas.FillRect(RectF(FLineLeft[pi1][li1], st + FLineTop[pi1][li1], FCharRight[pi1][li1].Last,
          st + FLineTop[pi1][li1 + 1]) * RectF(0, 0, Width, Height), 0, 0, [], 0.5, bb);
    ...
    begin
      if li1 = FLineTop[pi1].Count - 1 then
        Canvas.FillRect(RectF(FCharRight[pi1][li1][ci1 - 1], st + FLineTop[pi1][li1],
          FCharRight[pi1][li1].Last, st + FParaEnd[pi1]) * RectF(0, 0, Width, Height), 0, 0, [], 0.5, bb)
      else
        Canvas.FillRect(RectF(FCharRight[pi1][li1][ci1 - 1], st + FLineTop[pi1][li1],
          FCharRight[pi1][li1].Last, st + FLineTop[pi1][li1 + 1]) * RectF(0, 0, Width, Height),
          0, 0, [], 0.5, bb);
    end;
  pt := pi1;
  tl := li1 + 1;
  ...
  begin
    ...
    begin
      if tl = FLineTop[pt].Count - 1 then
        Canvas.FillRect(RectF(FLineLeft[pt][tl], st + FLineTop[pt][tl], FCharRight[pt][tl].Last,
  ...
  begin
    if li2 = FLineTop[pi2].Count - 1 then
      Canvas.FillRect(RectF(FLineLeft[pi2][li2], st + FLineTop[pi2][li2], FCharRight[pi2][li2][ci2 - 1],
...
begin
  ...
  if FParas[Index].Count = 0 then
    ...
    begin
      ...
      Clear(0);
      fi := (FParaSct[Index].FirstIndent + FParaSct[Index].IndentLevel * LevelFirst) * ts + Padding.Left;
      ri := (FParaSct[Index].RightOffset + FParaSct[Index].IndentLevel * Levelright) * ts + Padding.Right;
  ...
  li := (FParaSct[Index].LeftOffset + FParaSct[Index].IndentLevel * LevelLeft) * ts + Padding.Left;
  ri := (FParaSct[Index].RightOffset + FParaSct[Index].IndentLevel * Levelright) * ts + Padding.Right;
  if TMouseInfo.Wheeling in FMouseCon then
    ri := ri + 10;
...
begin
  ...
  begin
    ...
    Stream.Read(FParaSct[i]^.IndentLevel, SizeOf(Integer));
    Stream.Read(FParaSct[i]^.IsFolded, SizeOf(Boolean));
  end;
  // 读取文字信息
  Stream.Read(c, SizeOf(Integer));
  for i := 0 to c - 1 do
...
begin
  ...
  bb := TBrush.Create(TBrushKind.Solid, FFoldColor);
  ...
  try
    ...
    begin
      ShowFoldLine := False;
      if (LastLevel < FParaSct[i].IndentLevel) and (LastLevel > -1) then
      begin
        FParaSct[i].IsShowed := False;
        Continue;
      end
      else
      begin
        FParaSct[i].IsShowed := True;
        if FParaSct[i].IsFolded then
        begin
          LastLevel := FParaSct[i].IndentLevel;
          ShowFoldLine := True;
        end
        else
          LastLevel := -1;
      ...
      begin
        tb := tp + FParaHeight[i] - FScrollPos + Padding.Top + tt;
        Canvas.DrawBitmap(FParaMaps[i], RectF(0, FScrollPos - tp, Width, FParaHeight[i]),
          RectF(0, Padding.Top + tt, Width, tb), 1);
      end
      else if (tp >= FScrollPos) and (tp - FScrollPos + FParaHeight[i] <= Height - tg) then
      begin
        tb := tp - FScrollPos + FParaHeight[i] + Padding.Top + tt;
        Canvas.DrawBitmap(FParaMaps[i], RectF(0, 0, Width, FParaHeight[i]),
          RectF(0, tp - FScrollPos + Padding.Top + tt, Width, tb), 1);
      end
      else if (tp - FScrollPos < Height - Padding.Bottom) and (tp - FScrollPos + FParaHeight[i] > Height - tg) then
      begin
        tb := Height - Padding.Bottom;
        Canvas.DrawBitmap(FParaMaps[i], RectF(0, 0, Width, Height + FScrollPos - tp - tg),
          RectF(0, tp - FScrollPos + Padding.Top + tt, Width, tb), 1);
      end
      else
        ShowFoldLine := False;
      if ShowFoldLine then
        Canvas.FillRect(RectF(FLineLeft[i].Last, tb - 0.5 * FFoldWidth, FCharRight[i].Last.Last, tb + 0.5 * FFoldWidth),
          0, 0, [], 1, bb);
      tp := tp + FParaHeight[i];
    end;
    FPassHeight := tp;
...
begin
  ...
  for i := 0 to FParas.Count - 1 do
  begin
    if not FParaSct[i].IsShowed then
      Continue;
...
procedure TUIPassage.SetFoldColor(const Value: TAlphaColor);
begin
  if FFoldColor <> Value then
    FFoldColor := Value;
  Repaint;
end;

procedure TUIPassage.SetFoldWidth(const Value: Single);
begin
  if (FFoldWidth <> Value) and (Value >= 0)  then
    FFoldWidth := Value;
  Repaint;
end;
...
procedure TUIPassage.SetLevelFirst(const Value: Single);
begin
  if Value = FLevelFirst then
    Exit;
  if Value >= 0 then
    FLevelFirst := Value
  else
...
procedure TUIPassage.SetLevelLeft(const Value: Single);
begin
  if Value = LevelLeft then
    Exit;
  if Value >= 0 then
    FLevelLeft := Value
  else
...
procedure TUIPassage.SetLevelRight(const Value: Single);
begin
  if Value = LevelRight then
    Exit;
  if Value >= 0 then
    FLevelRight := Value
  else
...
begin
  ...
  begin
    ...
    Stream.Write(FParaSct[i].IndentLevel, SizeOf(Integer));
    Stream.Write(FParaSct[i].IsFolded, SizeOf(Boolean));
  end;
  // 储存文字信息
  c := FParas.Count;
  Stream.Write(c, SizeOf(Integer));
